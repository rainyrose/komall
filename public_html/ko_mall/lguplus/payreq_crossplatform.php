<?php
	...
	if($_SERVER['HTTPS']){
     ...
     /* 가상계좌(무통장) 결제 연동을 하시는 경우 아래 LGD_CASNOTEURL 을 설정하여 주시기 바랍니다. */
    $LGD_CASNOTEURL				= $HOST_.$_SERVER['HTTP_HOST']."/ko_mall/lguplus/cas_noteurl.php";

    /* LGD_RETURNURL 을 설정하여 주시기 바랍니다. 반드시 현재 페이지와 동일한 프로트콜 및  호스트이어야 합니다. 아래 부분을 반드시 수정하십시요. */
    $LGD_RETURNURL				= $HOST_.$_SERVER['HTTP_HOST']."/ko_mall/lguplus/returnurl.php";
    $configPath                 = $_SERVER['DOCUMENT_ROOT']."/../lguplus";                                  //LG유플러스에서 제공한 환경파일("/conf/lgdacom.conf") 위치 지정.
...
function payment_return() {
	...
	if (fDoc.document.getElementById('LGD_RESPCODE').value == "0000") {
			...
			document.getElementById("LGD_PAYINFO").action = "/ko_mall/product/user/proc.php";
			document.getElementById("LGD_PAYINFO").submit();
	} else {
		$("[data-form-append]").remove();
...
<?php
  foreach ($payReqMap as $key => $value) {
   ...
   ?>
	<script type="text/javascript">
		$("form[name=LGD_PAYINFO]").append("<input type='hidden' name='"+"<?=$key?>"+"' id='"+"<?=$key?>"+"' value='"+"<?=$value?>"+"' data-form-append>");
	</script>
   <?
  }
//  var_dump($_SESSION);
