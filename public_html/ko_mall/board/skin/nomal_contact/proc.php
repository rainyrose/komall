...
$board = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM board_config WHERE id='$board_id' LIMIT 1"));
$view_count = 0;
$ip = $REMOTE_ADDR;
...
if($mode == "modify_proc"){
	...
	alert("수정되었습니다.");
	$_SESSION[this_info] = hash("sha256", $no);
	url($return_url);
	...
	$check_ = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM $board_id WHERE ref_no='$ref_no' AND depth > 0 ORDER BY ref_group DESC LIMIT 1"));
