...
if($mode == "modify_proc"){
	...
	if(!$is_admin){
		...
		if($check_ > 0) {
			error("하위에 등록된 답변글이 있어 삭제할수 없습니다.");
			exit;
		}
	...
	if($is_admin){
		...
		url("/member/member.html?board_id=board_review&mode=review");
	}
...
if($_SESSION['auth_level'] != "1"){
	$chk_query = "SELECT * FROM koweb_product WHERE no='{$category}'";
	$chk_result = mysqli_query($connect, $chk_query);
	$chk_row = mysqli_num_rows($chk_result);
	$chk_product = mysqli_fetch_array($chk_result);

	if($chk_row == 0) error("잘못된 접근입니다.(상품정보가 없습니다)");

	$chk_query = "SELECT * FROM koweb_order WHERE member='{$_SESSION['member_id']}' AND ref_product='{$chk_product['id']}' AND (state='주문완료' OR state='배송완료' OR state='반품완료' OR state='취소완료') AND order_id='{$order_id}'";
	$chk_result = mysqli_query($connect, $chk_query);
	$order_rows = mysqli_num_rows($chk_result);

	$chk_query = "SELECT * FROM $board_id WHERE id='{$_SESSION['member_id']}' AND category='{$category}' AND order_id='{$order_id}'";
	$chk_result = mysqli_query($connect, $chk_query);
	$board_rows = mysqli_num_rows($chk_result);

	if(($order_rows - $board_rows) <= 0) error("해당상품에 후기를 남길수 없습니다.");
}
