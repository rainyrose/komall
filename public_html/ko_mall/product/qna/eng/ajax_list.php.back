...
<p class="title">Q&A</p>
<span class="sub">상품에 대해 궁금한 점을 해결해 드립니다.</span>
    ...
    <li>
        ...
        <div class="thead">
            ...
            <span data-shop-accordion="status">
                ...
                ?>
                <!-- <i class="status01">답변대기</i> -->
                <i class="<?=$reply_count==0 ? "status01" : "status02"?>"><?=$reply_count==0 ? "답변대기" : "답변완료"?></i>
            </span>
            <!-- 제목 -->
            <a href="#" data-shop-accordion="subject"><? if($row['secret'] == "Y"){ ?><i class="locked">비밀글</i><? } ?><?=$row['title']?></a>
            <!-- 아이디 -->
        ...
        <? if($row['secret'] == "Y" && $qna_config['auth_comment'] < $_SESSION['auth_level'] && !in_array($row['no'],$_SESSION['qna_secret'])){ ?>
            <div class="tbody">
                <div class="shop_password">
                    ...
                    <a href="javascript:void(0)" onclick="check_password('<?=$row['no']?>',this)" class="button gray">확인</a>
                </div>
            </div>
        <? }else{ ?>
        <!-- 내용 -->
...
<div class="btn_area tar">
    <? if($auth_write){ ?>
    <a href="<?=$add_folder?>/board/board.html?board_id=<?=$board_id?>&mode=write&id=<?=$no?>" class="button">문의하기</a>
    <? } ?>
    <a href="<?=$add_folder?>/board/board.html?board_id=<?=$board_id?>" class="button white">모두보기</a>
</div>
...
<div class="pagination">
    ...
    <a href="javascript:void" onclick='getAjaxList("0","<?=$no?>","<?=$board_id?>","qna","shopView04")' class="btn_first">맨처음</a>

    <? if ($start + $scale >  $scale * $page_scale) {
        $pre_start = $start - $scale * $page_scale; ?>
    <a href="javascript:void" onclick='getAjaxList("<?=$pre_start?>","<?=$no?>","<?=$board_id?>","qna","shopView04")' class="btn_prev">이전</a>
    <? } ?>
    ...
    if ($total > (($page + 1) * $scale * $page_scale)) {
        ...
        <a href="javascript:void" onclick='getAjaxList("<?=$n_start?>","<?=$no?>","<?=$board_id?>","qna","shopView04")' class="btn_next">다음</a>
    <? }
    ...
    ?>
    <a href="javascript:void" onclick='getAjaxList("<?=$end_start?>","<?=$no?>","<?=$board_id?>","qna","shopView04")' class="btn_last">맨마지막</a>
</div>
<!-- //페이징 -->
