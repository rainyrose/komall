...
if($row['password'] == $password){ ?>
    <? $_SESSION['qna_secret'][] = $no; ?>
        ...
        <div class="btn">
            <a href="/board/board.html?board_id=<?=$board_id?>&mode=modify&no=<?=$row['no']?>" class="button sm">글수정</a>
            <a href="javascript:void(0)" onclick="if(confirm('복구할수 없습니다. 정말로 삭제하시겠습니까?'))window.location.href='/board/board.html?board_id=<?=$board_id?>&mode=delete&no=<?=$row['no']?>'" class="button sm white">글삭제</a>
        </div>
        <? } ?>
        <?
        $ref_board_no = $row['no'];
