...
$d_options = $direct_option_name."|".$direct_option;
...
if($product['max_count']){
	if($product['max_count'] < $this_count){
	    $result['ment'] = "이 상품은 최대 ".($product['max_count']+1)."개 이상 장바구니에 담을수 없습니다.[현재 장바구니 : {$cart['cnt']}개]";
		$result['flag'] = "STOP";
	    echo json_encode($result);
	    exit;
	}
...
if($option_flag == "Y"){
    ...
    if($error_flag){
        $result['flag'] = "OK";
        $result['ment'] = "찾을수없는 옵션이 있어, 해당 상품을 제외하고 장바구니에 담았습니다.";
        echo json_encode($result);
        exit;
    }
}else{
