...
foreach ($product_id_col as $key => $product_id) {
    ...
    if($cart_num_row == 0){
    	...
    	foreach ($cart_add_option_list as $key2 => $add_option) {
    	...
    	foreach ($post_add_option_list as $key2 => $add_option) {
    		list($add_option_id,$add_option_cnt) = explode("|",$add_option);
    		$cart_add_option_col[$add_option_id] = $cart_add_option_col[$add_option_id] + $add_option_cnt;
    	}

    	$tmp_add_option_list;
    	foreach ($cart_add_option_col as $key2 => $value) {
    		if($key2 == "")continue;
    		if($value == "")continue;
    		$tmp_add_option_list[] = $key2."|".$value;
    	}
