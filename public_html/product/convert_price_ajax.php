...
foreach($options_json AS $json){
	...
	foreach($options_data AS $odata){
		...
		if($mode == "WON"){
			$options_price[option][$i][option_detail_price] = $option_detail_price;
			$options_price[option][$i][option_price] = $option_price;
		} else if($mode == "USD"){
	...
	foreach($add_options_data AS $aodata){
		...
		if($mode == "WON"){
			$options_price[add][$j][add_option_detail_price] = $add_option_detail_price;
			$options_price[add][$j][add_option_price] = $add_option_price;
		} else {
...
$query = "SELECT * FROM koweb_add_delivery_price WHERE start_zip <= '$zip' AND end_zip >= '$zip' ORDER BY price DESC LIMIT 1";
$tquery = "SELECT * FROM koweb_add_delivery_price WHERE start_zip <= '$zip' AND end_zip >= '$zip'";
...
if($mode == "WON"){
	//총구매금액
	$options_price[product_total] = $price_check;
	//배송비
	$options_price[delivery_price] = get_delivery_price($connect, $product_id_array, $price_check);
	//추가배송비
	if($check != 0){
		$options_price[delivery_add_price] = $row[price];
	} else {
		$options_price[delivery_add_price] = 0;
	}

	$options_price[total] = $options_price[product_total] + $options_price[delivery_price] + $options_price[delivery_add_price];

} else {
	//총구매금액
	$options_price[product_total] = number_format($price_check / $site_pay[transfer], 2);
	//배송비
	$options_price[delivery_price] = number_format(get_delivery_price($connect, $product_id_array, $price_check)  / $site_pay[transfer], 2 );
	//추가배송비
	if($check != 0){
		$options_price[delivery_add_price] = number_format($row[price] / $site_pay[transfer], 2);
	} else {
		$options_price[delivery_add_price] = 0;
	}
	$options_price[total] = $options_price[product_total] + $options_price[delivery_price] + $options_price[delivery_add_price];
