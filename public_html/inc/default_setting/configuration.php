...
if($mid != "member"){
	switch($menu_default[use_type]){
		case "content" :
			...
			if($content_default[content_type] == "product"){
				url("/product/product.html?mid=$mid&category=".$content_default[ref_product]);

			} else {
				$return = $_SERVER[DOCUMENT_ROOT]."/".$content_default[category] . "/" . $content_default[content_type]."/".$content_default[content_type].".php";
				include_once $return;
			}
			break;
		...
		case "link" :
			...
			if($link_menu_id[category] == "default"){
				url("/contents/".$link_menu_id[dir]."/page.html?mid=$menu_default[link_menu_id]");
			} else {
				url("/".$link_menu_id[category] . "/contents/".$link_menu_id[dir]."/page.html?mid=$menu_default[link_menu_id]");
			}
			break;
	}
} else{
